...
class LogisticRegression:
    ...
    def GDfit(self):
        for i in range(self.n_iters):
            yPred = 1 / \
                (1 + np.exp(- (self.xTrain.dot(self.weights) + self.bias)))
    ...
    def SGDfit(self):
        for i in range(self.n_epochs):
            indices = np.random.choice(
                self.xTrain.shape[0], self.batch_size, replace=False)
            xtrain = self.xTrain[indices]
            ytrain = self.yTrain[indices]
            for j in range(self.batch_size):
                yPred = 1 / \
                    (1 + np.exp(- (xtrain.dot(self.weights) + self.bias)))
    ...
    def addMetrics(self):
        h_xTest = 1 / \
            (1 + np.exp(- (self.xTest.dot(self.weights) + self.bias)))
        h_xTest = np.array([pow(10, -7) if i == 0 else i for i in h_xTest])
        h_xTest = np.array(
            [(1 - pow(10, -7)) if i == 1 else i for i in h_xTest])
        loss = np.dot(- self.yTest,  np.log(h_xTest)) - \
            np.dot(1 - self.yTest, np.log(1 - h_xTest))
    ...
    def plotter(self,
        ...
        if sgd:
            if loss:
                ...
                plt.title('Stochastic Gradient Descent ; Loss ; lr=' +
            ...
            if acc:
                ...
                plt.title(
                    'Stochastic Gradient Descent ; Accuracy ; lr=' + str(self.learning_rate))
                plt.legend()
                plt.xlabel('Number of Iterations')
                plt.show()
            if precision:
                plt.plot(iters, self.precision[:-1],
            ...
            plt.legend()
            plt.title('Stochastic Gradient Descent ; lr=' +
                      str(self.learning_rate))
            plt.xlabel('Number of Iterations')
            plt.show()
        else:
            if loss:
                plt.plot(iters, self.loss[:-1], color='r', label='Loss')
                plt.title('Gradient Descent ; Loss ; lr=' +
                          str(self.learning_rate))
                plt.legend()
                plt.xlabel('Number of Iterations')
                plt.show()
            if acc:
                plt.plot(iters, self.accuracy[:-1],
                         color='g', label='Accuracy')
                plt.title('Gradient Descent ; Accuracy ; lr=' +
                          str(self.learning_rate))
                plt.legend()
                plt.xlabel('Number of Iterations')
                plt.show()
...
dataLocation = 'dataset_LR.csv'
for rates in [0.001, 0.01, 0.1]:
    model = LogisticRegression(dataLocation, lr=rates)
    sgd_acc_sum, gd_acc_sum, sgd_loss_sum, gd_loss_sum = 0, 0, 0, 0
    print('{:*^95}'.format(
        '10 random 70:30 train test splits with learning rate ' + str(rates)))
    for i in range(10):
        model.setData()
        model.fit(sgd=False)
        if i == 0:
            model.plotter(sgd=False)
            print('{:<45}{:>45}'.format('Model weights(Gradient Descent):', str(model.weights)))
        ...
        if i == 0:
            ...
            print('{:<45}{:>45}'.format('Model weights(Stochastic Gradient Descent):', str(model.weights)))
        asgd, lsgd = model.accuracy[-1], model.loss[-1]
        sgd_acc_sum += asgd
        sgd_loss_sum += lsgd

        if i == 0:
            print('{:^5}{:^30}{:^30}{:^30}'.format('No.', 'Stat Name',
                  'Gradient Descent', 'Stochastic Gradient Descent'))

        print('{:^5}{:^30}{:^30}{:^30}'.format(i + 1, 'Accuracy', agd, asgd))
        print('{:^5}{:^30}{:^30}{:^30}'.format('', 'Loss', lgd, lsgd))

    print('{:^5}{:^30}{:^30}{:^30}'.format('All', 'Avg Accuracy', '{:.6f}'.format(
        gd_acc_sum / 10), '{:.6f}'.format(sgd_acc_sum / 10)))
    print('{:^5}{:^30}{:^30}{:^30}'.format('All', 'Avg Loss', '{:.6f}'.format(
        gd_loss_sum / 10), '{:.6f}'.format(sgd_loss_sum / 10)))
    print()
    print()
